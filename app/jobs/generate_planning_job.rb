class GeneratePlanningJob < ApplicationJob
  ...
  def perform(goal_id)
    ...
    time_slots = goal.time_slots.order(:day_of_week, :start_time)    # On récupère les instance de 'time_slot' que l'on transforme en array et qu'on classe par jour et heure
    steps_to_assign = goal.steps.where(session: nil).order(:priority).to_a     # on récupère toutes les étapes qui sont assigné à la session par défaut ou qui n'ont pas de session
    ...
    puts "time slots#{time_slots.map(&:day_of_week)}"
    puts "step to assign: #{steps_to_assign.map(&:description)} - #{steps_to_assign.length}"
    ...
    while steps_to_assign.any?
      ...
      while steps_total_time < session.total_time && steps_to_assign.any?
        ...
        unless step.update(session: session)
          puts "Erreur update step: #{step.errors.full_messages}"
        end
